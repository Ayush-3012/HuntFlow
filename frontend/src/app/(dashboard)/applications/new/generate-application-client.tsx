...
type GenerateApplicationClientProps = {
  initialJobId?: string;
};

export default function GenerateApplicationClient({ initialJobId }: GenerateApplicationClientProps) {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (initialJobId && latestFirstJobs.some((job) => job._id === initialJobId)) {
        ...
        <div className="bg-white border rounded-xl p-6 space-y-3">
          ...
          {result.coldMessage ? (
            <div className="bg-gray-50 border rounded-lg p-3">
              ...
              <p className="text-sm text-gray-700 whitespace-pre-wrap">{result.coldMessage.message}</p>
            </div>
          ) : null}
        </div>
      ) : null}
